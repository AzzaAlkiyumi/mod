...
export function RolesList({ initial }: { initial: RoleRow[] }) {
  ...
  const visible = useMemo(() => {
    ...
    const sorted = [...filtered].sort((a, b) => {
      ...
      if (sortKey === "users") return b._count.users - a._count.users;
  ...
  async function handleDelete() {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(json?.error?.formErrors?.[0] ?? s.deleteFailed);
      }
      ...
      toast.error(err instanceof Error ? err.message : s.deleteFailed);
    } finally {
      setDeleting(false);
    }
  }
              ...
              <TableBody>
                {visible.map((role) => (
                  <TableRow key={role.id}>
                    ...
                    <TableCell className="text-end">{role._count.users}</TableCell>
