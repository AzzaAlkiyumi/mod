...
export function POSView({
  ...
  async function handleCompleteSale() {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(json?.error?.formErrors?.[0] ?? t.pos.toasts.failed);
      }
      toast.success(t.pos.toasts.completed(json.data.number));
      setCart([]);
      setCustomer(null);
    } catch (err) {
      toast.error(err instanceof Error ? err.message : t.pos.toasts.failed);
    } finally {
      setSubmitting(false);
    }
  }
