...
export function TaxGroupsPanel({
  ...
  async function handleSave() {
    ...
    try {
      ...
      const res = isNew
        ...
        : await api.patch(`/tax-groups/${selectedId}`, payload);
      toast.success(isNew ? s.common.createdToast : s.common.updatedToast);
      setSelectedId(null);
      setItems((prev) => {
        const withoutOldDefault = form.isDefault
          ? prev.map((it) => ({ ...it, isDefault: it.id === res.data.data.id }))
          : prev;
        if (isNew) {
          return [...withoutOldDefault, res.data.data].sort((a, b) => a.name.localeCompare(b.name));
        }
        return withoutOldDefault.map((it) => (it.id === res.data.data.id ? res.data.data : it));
      });
    } catch (err) {
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    try {
      await api.delete(`/tax-groups/${deleteTarget.id}`);
      ...
      <Card className="overflow-hidden py-0">
        <div className="flex items-center justify-between px-4 pt-4 pb-3">
          ...
          <div className="flex items-center gap-2">
            <Link
              to="/admin/tax-management/classifications"
          ...
          {items.map((row) => (
            ...
            >
              ...
              <span className="text-xs text-muted-foreground">
                {s.groups.inUse((row.productsCount ?? 0) + (row.categoriesCount ?? 0))}
              </span>
              {!row.active && (
