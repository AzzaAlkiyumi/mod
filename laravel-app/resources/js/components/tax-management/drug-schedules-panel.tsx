...
const emptyForm = { shortCode: "", country: "", displayName: "", description: "", active: true };

export function DrugSchedulesPanel({ initial }: { initial: DrugScheduleRow[] }) {
  ...
  const [deleteTarget, setDeleteTarget] = useState<DrugScheduleRow | null>(null);
  ...
  function selectExisting(row: DrugScheduleRow) {
  ...
  async function handleSave() {
    if (!form.shortCode.trim() || !form.country.trim() || !form.displayName.trim()) {
    ...
    try {
      ...
      const res = isNew
        ? await api.post("/drug-schedules", payload)
        : await api.patch(`/drug-schedules/${selectedId}`, payload);
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    try {
      await api.delete(`/drug-schedules/${deleteTarget.id}`);
  ...
  return (
    <div dir={dir} className="flex flex-col gap-4">
      <div>
        <h1 className="text-2xl font-semibold tracking-tight">{s.drugSchedules.title}</h1>
        <p className="mt-1 text-sm text-muted-foreground">{s.drugSchedules.subtitle}</p>
      </div>
      <div className="grid gap-4 lg:grid-cols-[1fr_360px]">
      <Card className="overflow-hidden py-0">
        <div className="flex items-center justify-between px-4 pt-4 pb-3">
          <h2 className="text-sm font-semibold">{s.drugSchedules.countLabel(items.length)}</h2>
          <Button type="button" size="sm" onClick={selectNew}>
            <Plus className="size-4" /> {s.drugSchedules.newItem}
          </Button>
        </div>
        <div className="divide-y divide-border border-t border-border">
          {items.length === 0 && (
            <p className="px-4 py-8 text-center text-sm text-muted-foreground">
              {s.drugSchedules.empty}
            </p>
          )}
          {items.map((row) => (
            <button
        ...
        {selectedId && (
          <CardContent className="flex flex-col gap-4">
            <div className="flex flex-col gap-1.5">
              <Label htmlFor="ds-code">{s.drugSchedules.shortCode}</Label>
              <Input
                id="ds-code"
                ...
                dir="ltr"
              />
              <p className="text-xs text-muted-foreground">{s.drugSchedules.shortCodeHint}</p>
            </div>
            <div className="flex flex-col gap-1.5">
              <Label htmlFor="ds-country">{s.drugSchedules.country}</Label>
              <Input
                id="ds-country"
                value={form.country}
                onChange={(e) => setForm((f) => ({ ...f, country: e.target.value.toUpperCase() }))}
                placeholder={s.drugSchedules.countryPlaceholder}
                maxLength={2}
                dir="ltr"
                className="max-w-xs"
              />
            ...
            <div className="flex flex-col gap-1.5">
              <Label htmlFor="ds-display">{s.drugSchedules.displayName}</Label>
              <Input
                id="ds-display"
            ...
            <div className="flex flex-col gap-1.5">
              <Label htmlFor="ds-description">{s.drugSchedules.description}</Label>
              <Textarea
                id="ds-description"
            ...
            <label className="flex items-start gap-2.5">
              ...
              />
              <span>
                <span className="block text-sm font-medium">{s.common.active}</span>
                <span className="block text-xs text-muted-foreground">
                  {s.drugSchedules.activeHint}
                </span>
              </span>
            </label>
