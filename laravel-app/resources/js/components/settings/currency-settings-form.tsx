...
import { formatAmountWithConfig, setCurrencyFormat, type CurrencyFormat } from "@/lib/utils";
...
export function CurrencySettingsForm({ initial }: { initial: CurrencyFormat }) {
  ...
  async function handleSave() {
    ...
    setSaving(true);
    try {
      const res = await api.put("/settings/currency", {
      ...
      setCurrencyFormat(res.data.data);
      navigate(0);
    } catch (err) {
      toast.error(apiErrorMessage(err, s.saveFailed));
    } finally {
      setSaving(false);
    }
  }
