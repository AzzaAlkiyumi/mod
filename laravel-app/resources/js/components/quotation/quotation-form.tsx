...
export function QuotationForm({ initial }: { initial?: QuotationFormInitialData }) {
  ...
  useEffect(() => {
    api.get("/stores").then((res) => {
      ...
      if (!storeId && res.data.data?.[0]) setStoreId(res.data.data[0].id);
    });
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(async () => {
      ...
      if (requestId === totalsRequestId.current) {
        if (res.data.data) setTotals(res.data.data);
  ...
  async function handleSave() {
    ...
    try {
      ...
      const res = isEdit
        ? await api.put(`/quotations/${initial!.id}`, payload)
        : await api.post("/quotations", payload);

      toast.success(isEdit ? t.form.toasts.updated : t.form.toasts.savedAsDraft(res.data.data.number));
      navigate(`/admin/quotations/${res.data.data.id}`);
    } catch (err) {
      toast.error(apiErrorMessage(err, t.form.errors.saveFailed));
    } finally {
      setSaving(false);
    }
  }
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="flex shrink-0 gap-2">
          <Button variant="ghost" asChild>
            <Link to="/admin/quotations">{t.common.discard}</Link>
          </Button>
